...
class Auth
{
    public static function login(string $email, string $password, string $ip): array
    {
        ...
        if (Security::isRateLimited($ipHash, 'login')) {
            return ['success' => false, 'error' => 'Trop de tentatives. Reessaie dans quelques minutes.'];
        }
        ...
        if ($user['status'] === 'anonymized') {
            return ['success' => false, 'error' => 'Ce compte a ete supprime.'];
        }
        if (!$user['email_verified']) {
            return ['success' => false, 'error' => 'Verifie ton email avant de te connecter.'];
        }
    ...
    public static function getOAuthUrl(string $provider): string
    {
        ...
        return match ($provider) {
            ...
            default => throw new InvalidArgumentException('Provider inconnu'),
        };
    }
    ...
    public static function handleOAuthCallback(string $provider, string $code, string $state): array
    {
        if (!isset($_SESSION['oauth_state']) || $_SESSION['oauth_state'] !== $state) {
            return ['success' => false, 'error' => 'Etat OAuth invalide.'];
        }
        ...
        if (!$userData || empty($userData['email'])) {
            return ['success' => false, 'error' => 'Impossible de recuperer les informations du provider.'];
        }
    ...
    public static function logout(): void
    {
        ...
        session_destroy();
        if ($userId) {
            self::auditLog((int) $userId, 'logout', null);
        }
    }
    ...
    private static function loginOrRegisterOAuth(string $provider, array $data): array
    {
        ...
        self::auditLog((int) $userId, 'register', $provider);
    ...
    {
        return 'https://login.microsoftonline.com/' . OAUTH_MICROSOFT_TENANT . '/oauth2/v2.0/authorize?' . http_build_query([
    ...
    {
        $token = self::httpPost('https://login.microsoftonline.com/' . OAUTH_MICROSOFT_TENANT . '/oauth2/v2.0/token', [
            'client_id' => OAUTH_MICROSOFT_CLIENT_ID,
    ...
    private static function facebookCallback(string $code): ?array
    {
        ...
        $info = self::httpGet('https://graph.facebook.com/me?fields=id,name,email,picture', $token['access_token']);
        if (empty($info['id'])) {
            return null;
        }
